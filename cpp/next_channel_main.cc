...
public:
    ...
    for (const auto &entry : grouped_click_times) {
      ...
      for (size_t index = 0; index < group_click_times.size(); index++) {
        if (index + 1 < group_click_times.size()) {
          ...
          feature[group_click_times[index]] = channel[next_index];
        } else {
  ...
  virtual bool filter(uint16_t key_mask) {
    return ((key_mask & KEY_IP) && (key_mask & KEY_DEVICE) && (key_mask & KEY_OS) && ((key_mask & KEY_CHANNEL) == 0)) && (key_mask != KEY_MASK_ALL);
  }
};
