...
class ExactSameClickId : public GroupedFeatureCalculator<uint8_t, arrow::UInt16Type> {
public:
  ExactSameClickId(): GroupedFeatureCalculator() {}
  
  virtual string name() {
    return "ExactSameClickId";
  }
  ...
  virtual vector<uint8_t> calculate_feature(const unordered_map<uint64_t, vector<size_t>> &grouped_click_times) {
    ...
    for (const auto &entry : grouped_click_times) {
      ...
      for (size_t index = 0; index < group_click_times.size(); index++) {
        future_click_count[group_click_times[index]] = index;
      }
    }
...
int main(int argc, char **argv)
{
  ...
  ExactSameClickId feature_calculator;
