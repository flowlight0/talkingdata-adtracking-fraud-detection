...
class PastClickCount : public GroupedFeatureCalculator<uint16_t, arrow::UInt16Type> {
public:
  PastClickCount(uint64_t window_size_in_seconds): GroupedFeatureCalculator(), window_size_in_nanoseconds(window_size_in_seconds * 1000000000ULL) {}
  ...
  virtual vector<uint16_t> calculate_feature(const unordered_map<uint64_t, vector<size_t>> &grouped_click_times) {
    vector<uint16_t> past_click_count(ip.size());
    for (const auto &entry : grouped_click_times) {
      ...
      for (size_t index = 0; index < group_click_times.size(); index++) {
        ...
        past_click_count[group_click_times[index]] = min<int>(index - cursor + 1, numeric_limits<uint16_t>::max());
      }
    }
    return past_click_count;
  }
...
int main(int argc, char **argv)
{
  ...
  PastClickCount feature_calculator(atoi(argv[7]));
